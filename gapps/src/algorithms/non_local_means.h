...
std::map<std::string, Func> non_local_means(
    ...
    feature(x, y, cf) = 0.f;
    feature(x, y, cf) += clamped(x + r_ff.x - feature_filter.width()  / 2,
                                 y + r_ff.y - feature_filter.height() / 2,
                                 r_ff.z) * feature_filter_func(r_ff.x, r_ff.y, r_ff.z, cf);

    Var dx("dx"), dy("dy");
    Func dc("d");
    dc(x, y, dx, dy, cf) = pow(feature(x, y, cf) - feature(x + dx, y + dy, cf), 2);
    ...
    Func blur_d("blur_d");
    blur_d(x, y, dx, dy) = 0.f;
    blur_d(x, y, dx, dy) += d(x + patch_dom.x - patch_filter.width()  / 2,
                              y + patch_dom.y - patch_filter.height() / 2,
                              dx,
                              dy) * patch_filter_func(patch_dom.x, patch_dom.y);
    ...
    w(x, y, dx, dy) = exp(blur_d(x, y, dx, dy) * inv_sigma_sq);
