...
int bilateral_layer_forward_(THFloatTensor *input,
  ...
  int bias_depth = THFloatTensor_size(bias, 0);
  int bias_ci    = THFloatTensor_size(bias, 1);
  ...
  THArgCheck(
      bias_ci == channels_in, 0,
      "bias (%d) and input (%d) should have same batch_size", bias_ci, batch_size);
  THArgCheck(
      filt_depth == bias_depth, 0,
      "filter (%d) and bias (%d) should have same slice size", filt_depth, bias_depth);  
  ...
  bias   = THFloatTensor_newContiguous(bias);
  ...
  Buffer<float> bias_buf   = wrap(bias);
  Buffer<float> output_buf = wrap(output);

  // Run Halide code
  bilateral_layer_forward(input_buf, guide_buf, filter_buf, bias_buf, output_buf);
