...
class DeconvCG(nn.Module):
  def __init__(self,
    ...
    self.reg_kernels.data[:, 4, reg_kernel_center, reg_kernel_center + 1] = 1.0
    if not ref:
      self.reg_kernels.data[:, 5:, :, :].normal_(0, 0.02)
    ...
    if (self.reg_kernel_weights.shape[0] > 2):
      self.reg_kernel_weights.data[2:, :] = 0.1
    if ref:
    ...
    for b in range(num_batches):
      ...
      for stage in range(self.num_stages):
        ...
        for irls_it in range(num_irls_iter):
          ...
          num_iter = num_cg_iter
          if stage == self.num_stages - 1 and (not self.ref):
            num_iter *= 2
          for cg_it in range(num_iter):
            xrp = funcs.DeconvCGIter.apply(
                    xrp,
                    kernel,
                    self.reg_kernel_weights[stage + 1, :],
